...
if ($ConfigPath -eq "") {
    $ConfigPath = Join-Path $OptimizationDir "configs\experiment_config.json"
} elseif (-not [System.IO.Path]::IsPathRooted($ConfigPath)) {
    $ConfigPath = Join-Path $OptimizationDir $ConfigPath
}

if ($ResultsDir -eq "") {
    $ResultsDir = Join-Path $OptimizationDir "results"
} elseif (-not [System.IO.Path]::IsPathRooted($ResultsDir)) {
    $ResultsDir = Join-Path $OptimizationDir $ResultsDir
}
...
foreach ($config in $configs) {
    ...
    Invoke-CapturedCommand -Command @(
        "python", (Join-Path $OptimizationDir "parse_quartus_reports.py"),
...
Invoke-CapturedCommand -Command @(
    "python", (Join-Path $OptimizationDir "collect_results.py"),
    "--runs-dir", $runsDir,
